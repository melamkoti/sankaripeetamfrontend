import DonationImg1 from "../../../assets/images/donate-1.jpeg";
import DonationImg2 from "../../../assets/images/donate-2.jpeg";
import DonationImg3 from "../../../assets/images/donate-3.jpeg";
import DonationImg4 from "../../../assets/images/donate-4.jpeg";
import DonationImg5 from "../../../assets/images/donate-5.jpeg";
import DonationImg6 from "../../../assets/images/donate-6.png";
import Virkshaseva from "../../../assets/images/vrukshaseva.png"
export const DonationsCardData = [
  {
    id: 1,
    img: DonationImg1,
    title: "Annadanam",
    content:
      "Annadaanam is a sacred tradition of donating cooked food to those in need, reflecting the spirit of compassion and generosity. By offering meals, we nourish not only the body but also the soul, creating a deep connection between the giver and the receiver.",
  },
  {
    id: 2,
    img: DonationImg2,
    title: "TempleSeva",
    content:
      "Temple seva is a profound act of service where devotees contribute their time and efforts to maintain the sanctity and operations of the temple. Through seva, individuals connect with the divine, expressing their devotion and humility while supporting the spiritual well-being of the community.",
  },
  {
    id: 3,
    img: DonationImg3,
    title: "GoSeva",
    content:
      "Cow seva, or the service of caring for cows, is a revered practice that honors the sacredness of the cow in many traditions. By providing for these gentle creatures, devotees express their gratitude and respect, fostering a deep connection with nature and upholding the values of compassion and selfless service.",
  },
  {
    id: 4,
    img: DonationImg4,
    title: "Orphans",
    content:
      "Supporting orphans through seva is an act of deep compassion, providing care, education, and love to children who have lost their families. By participating in this noble service, we offer hope and a brighter future to those who need it most, ensuring they feel valued and nurtured.",
  },
  {
    id: 5,
    img: DonationImg5,
    title: "VivekaVidyalaya",
    content:
      "Viveka Vidyalaya is dedicated to providing holistic education, fostering academic excellence, and nurturing moral values in students. Through a balanced curriculum, the school aims to develop well-rounded individuals equipped with the knowledge, skills, and character needed to succeed in life and contribute positively to society.",
  },
  {
    id: 6,

    img: DonationImg6,
    title: "BhuSeva",
    content:
      "Soil rejuvenation involves restoring the health and fertility of soil through sustainable practices such as organic farming, crop rotation, composting, and reducing chemical inputs. By focusing on enriching the soil's natural nutrients and promoting biodiversity, soil rejuvenation ensures long-term agricultural productivity, combats land degradation, and supports a balanced ecosystem.",
  },
  {
    id: 7,

    img: Virkshaseva,
    title: "VrukshaSeva",
    content:
      "Vruksha Seva translates to 'tree service' or 'tree care,' emphasizing the protection and nurturing of trees as a form of environmental and spiritual duty."
  }
];
