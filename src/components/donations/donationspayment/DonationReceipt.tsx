...
const DonationReceipt = ({
  ...
  const DontaionReceiptPostService = UserModuleAPI.DonationReceiptPost;

  useEffect(() => {
    const fetchReceipt = async () => {
      try {
        const res = await fetch(DontaionReceiptPostService, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ donationId: donationData?.donationId }), // must pass donationId
        });

        if (!res.ok) throw new Error("Failed to fetch receipt");

        const receipt = await res.json();
        setReceiptId(receipt.receiptCode);
      } catch (err) {
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      const file = new File([blob], "Donation_Receipt.pdf", {
      ...
      alert("Failed to share receipt. Try downloading instead.");
    }
  };
      ...
      >
        ...
        <div className="p-3 sm:p-4 border-t border-gray-200 flex justify-center gap-3 sm:gap-4">
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.95 }}
            onClick={onClose}
            className="px-4 sm:px-6 py-2 bg-gray-200 text-gray-800 rounded-lg shadow text-xs sm:text-base"
          >
            Close
          </motion.button>
        </div>
