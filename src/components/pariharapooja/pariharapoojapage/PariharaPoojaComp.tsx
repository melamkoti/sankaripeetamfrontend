    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 p-5 md:p-24 gap-10 shadow-lg">
        {PariharaPoojaData.map((item, index) => {
          return (
            <div
              className="flex items-start gap-4 p-4 md:p-6 bg-white rounded-2xl shadow-md hover:shadow-xl transition-shadow duration-300 border border-orange-100"
              key={index}
            >
              {/* Icon Section */}
              <div className="flex-shrink-0 w-12 h-12 md:w-16 md:h-16 rounded-full bg-orange-50 flex items-center justify-center shadow-inner">
                <img
                  src={lampImg}
                  alt="lamp"
              ...
              <div className="flex flex-col gap-2">
                <h1 className="text-orange-700 font-semibold text-base md:text-lg tracking-wide">
                  {index + 1}. {item.title}
                </h1>
      ...
      <p className="italic text-red-600 text-center p-4 semibold text-1xl bg-red-50">
        <span className="italic text-red-500 text-1xl bold">NOTE :</span> " puja
