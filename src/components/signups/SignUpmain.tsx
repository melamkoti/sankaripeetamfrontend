...
function SignUpMain() {
  ...
  const UserSignUpService = UserModuleAPI.RegisterPost; // API endpoint for email signup
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  const onSubmit = async (data: FormFields) => {
    try {
      const response = await axios.post(UserSignUpService, data);
          ...
          >
            <div className="flex flex-col gap-2 relative">
              ...
              <input
                ...
                id="email"
                className="w-full border-2 outline-none border-slate-400 focus:border-[#FFA12B]  rounded-lg p-3 bg-[#F7F6F4] text-[#939393]"
              />
              {errors.email && (
